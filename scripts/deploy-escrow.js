// Compile + deploy KickoffEscrow on Hedera EVM.
...
const source = readFileSync('contracts/KickoffEscrow.sol', 'utf8');
const input = {
  language: 'Solidity',
  sources: { 'KickoffEscrow.sol': { content: source } },
  settings: { optimizer: { enabled: true, runs: 200 }, outputSelection: { '*': { '*': ['abi', 'evm.bytecode.object'] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const errs = (output.errors || []).filter((e) => e.severity === 'error');
if (errs.length) { errs.forEach((e) => console.error(e.formattedMessage)); process.exit(1); }
const artifact = output.contracts['KickoffEscrow.sol'].KickoffEscrow;
mkdirSync('contracts/artifacts', { recursive: true });
writeFileSync('contracts/artifacts/KickoffEscrow.json', JSON.stringify({ abi: artifact.abi, bytecode: '0x' + artifact.evm.bytecode.object }, null, 2));
...
console.log('KickoffEscrow deployed at:', address);

let env = readFileSync('.env', 'utf8');
const k = 'ESCROW_CONTRACT_ADDRESS';
env = env.includes(`${k}=`) ? env.replace(new RegExp(`^${k}=.*$`, 'm'), `${k}=${address}`) : env + `${k}=${address}\n`;
writeFileSync('.env', env);
console.log('Wrote ESCROW_CONTRACT_ADDRESS to .env');
